...
class Product_tags extends Admin_controller {
  ...
  public function index ($offset = 0) {
    ...
    $tags = ProductTag::find ('all', array (
        'offset' => $offset,
        'limit' => $limit,
        'order' => 'sort DESC',
  ...
  public function create () {
    ...
    $posts['sort'] = ProductTag::count ();

    $create = ProductTag::transaction (function () use ($posts) {
      if (!(verifyCreateOrm ($tag = ProductTag::create (array_intersect_key ($posts, ProductTag::table ()->columns)))))
        return false;
  ...
  public function update () {
    ...
    $tag = $this->tag;
    $update = ProductTag::transaction (function () use ($tag, $posts, $cover) {
      if (!$tag->save ())
        return false;
  ...
  public function sort ($id, $sort) {
    ...
    ProductTag::addConditions ($conditions, 'sort = ?', $this->tag->sort);

    $tag = $this->tag;
    
    $update = ProductTag::transaction (function () use ($conditions, $tag, $sort) {
      if (($next = ProductTag::find ('one', array ('conditions' => $conditions))) && (($next->sort = $sort) || true))
