<form action='<?php echo base_url ('admin', 'work_tags');?>' method='get' class="search<?php echo $has_search = array_filter (column_array ($columns, 'value')) ? ' show' : '';?>">
<?php 
  if ($columns) { ?>
...
<?php 
  ...
  <a href='<?php echo base_url ('admin', 'work_tags', 'add');?>'>新增</a>
</form>
        ...
        foreach ($tags as $tag) { ?>
          <tr>
            ...
            <td data-title='編輯' width='100'>
              ...
              <a href='<?php echo base_url ('admin', 'work_tags', $tag->id, 'tags');?>' class='icon-tags'></a>
              <a href='<?php echo base_url ('admin', 'work_tags', $tag->id, 'edit');?>' class='icon-pencil2'></a>
              <a href='<?php echo base_url ('admin', 'work_tags', $tag->id);?>' data-method='delete' class='icon-bin destroy'></a>
            </td>
            <td data-title='排序' width='50' class='sort'>
              <a href='<?php echo base_url ('admin', 'work_tags', $tag->id, 'sort', 'up');?>' data-method='post' class='icon-triangle-up'></a>
              <a href='<?php echo base_url ('admin', 'work_tags', $tag->id, 'sort', 'down');?>' data-method='post' class='icon-triangle-down'></a>
            </td>
          </tr>
  <?php }
      } else { ?>
