<form action='<?php echo base_url ('admin', 'article_tags');?>' method='get' class="search<?php echo $has_search = array_filter (column_array ($columns, 'value')) ? ' show' : '';?>">
<?php 
  if ($columns) { ?>
...
<?php 
  ...
  <a href='<?php echo base_url ('admin', 'article_tags', 'add');?>'>新增</a>
</form>
        ...
        foreach ($tags as $tag) { ?>
          <tr>
            ...
            <td data-title='編輯' width='70'>
              <a href='<?php echo base_url ('admin', 'article_tags', $tag->id, 'edit');?>' class='icon-pencil2'></a>
              <a href='<?php echo base_url ('admin', 'article_tags', $tag->id);?>' data-method='delete' class='icon-bin destroy'></a>
            </td>
          </tr>
  <?php }
      } else { ?>
